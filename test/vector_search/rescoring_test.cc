...
SEASTAR_TEST_CASE(index_option_rescoring_factor_valid_values) {
        ...
        co_await do_with_cql_env(
                [factor](cql_test_env& env) -> future<> {
                    ...
                    co_await env.execute_cql(
                            fmt::format("CREATE INDEX idx ON ks.cf (embedding) USING 'vector_index' WITH OPTIONS={{'rescoring_factor': {}}};", factor));
                },
                make_config());
    }
}

SEASTAR_TEST_CASE(index_option_rescoring_factor_invalid_value_below_range) {
    co_await do_with_cql_env(
            [](cql_test_env& env) -> future<> {
                auto schema = co_await create_test_table(env, "ks", "cf");

                BOOST_REQUIRE_THROW(
                        co_await env.execute_cql("CREATE INDEX idx ON ks.cf (embedding) USING 'vector_index' WITH OPTIONS={'rescoring_factor': 0.9};"),
                        exceptions::invalid_request_exception);
            },
            make_config());
}

SEASTAR_TEST_CASE(index_option_rescoring_factor_invalid_value_above_range) {
    co_await do_with_cql_env(
            [](cql_test_env& env) -> future<> {
                auto schema = co_await create_test_table(env, "ks", "cf");

                BOOST_REQUIRE_THROW(
                        co_await env.execute_cql("CREATE INDEX idx ON ks.cf (embedding) USING 'vector_index' WITH OPTIONS={'rescoring_factor': 100.1};"),
                        exceptions::invalid_request_exception);
            },
            make_config());
}

SEASTAR_TEST_CASE(rescoring_factor_multiplies_limit_for_vector_store_query) {
    ...
    co_await do_with_cql_env(
            [&](cql_test_env& env) -> future<> {
                ...
                co_await env.execute_cql("CREATE INDEX idx ON ks.cf (embedding) USING 'vector_index' WITH OPTIONS={'rescoring_factor': 3.4}");
                ...
                BOOST_CHECK_EQUAL(
                        parse_limit(server->ann_requests().back().body), 11); // ceil(${rescoring_factor} * ${limit}) = ceil(3.4 * 3) = ceil(10.2) = 11
            },
...
SEASTAR_TEST_CASE(oversampled_vector_store_results_are_limited_to_cql_limit) {
    ...
    co_await do_with_cql_env(
            [&](cql_test_env& env) -> future<> {
                ...
                co_await env.execute_cql("CREATE INDEX idx ON ks.cf (embedding) USING 'vector_index' WITH OPTIONS={'rescoring_factor': 2}");
    ...
    co_await do_with_cql_env(
            [&](cql_test_env& env) -> future<> {
                ...
                co_await env.execute_cql("CREATE INDEX idx ON ks.cf (embedding) USING 'vector_index' WITH OPTIONS={'rescoring_factor': 2, 'quantization': "
                                         "'b1', 'similarity_function': 'euclidean'}");
                // Insert test data with clearly increasing distances from [0,0,0]
                co_await env.execute_cql("INSERT INTO ks.cf (id, embedding) VALUES (1, [1, 0, 0])"); // Distance from [0,0,0] = 1.0
                co_await env.execute_cql("INSERT INTO ks.cf (id, embedding) VALUES (2, [2, 0, 0])"); // Distance from [0,0,0] = 4.0
                co_await env.execute_cql("INSERT INTO ks.cf (id, embedding) VALUES (3, [3, 0, 0])"); // Distance from [0,0,0] = 9.0
                co_await env.execute_cql("INSERT INTO ks.cf (id, embedding) VALUES (4, [4, 0, 0])"); // Distance from [0,0,0] = 16.0
                ...
                auto msg = co_await env.execute_cql("SELECT id FROM ks.cf ORDER BY embedding ANN OF [0, 0, 0] LIMIT 2;");
