...
SEASTAR_TEST_CASE(rescoring_factor_multiplies_limit_for_vector_store_query) {
    ...
    co_await do_with_cql_env(
            [&](cql_test_env& env) -> future<> {
                ...
                co_await env.execute_cql(
                        "CREATE INDEX idx ON ks.cf (embedding) USING 'vector_index' WITH OPTIONS={'rescoring_factor': 3, 'quantization': 'b1'}");
                ...
                BOOST_CHECK_EQUAL(parse_limit(server->ann_requests().back().body), 9); // ${rescoring_factor} * ${limit} = 3 * 3 = 9
            },
